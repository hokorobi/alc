...
func main() {
	...
	doc.Find(".ex_sentence, .kana").Each(func(_ int, ex *goquery.Selection) {
		ex.Remove()
	})
	...
	doc.Find("#resultsList").Each(func(_ int, result *goquery.Selection) {
		result.Find(".midashi, .midashi_je").Each(func(i int, midashi *goquery.Selection) {
			if begin <= i+1 && i+1 <= end {
				fmt.Fprintln(stdOut, RED+midashi.Text()+END)
				content := midashi.Next()
				content.Find(".attr").Each(func(_ int, attr *goquery.Selection) {
				...
				if content.Is("div") {
					...
					if wordclass.Size() > 0 {
						wordclass.Each(func(_ int, w *goquery.Selection) {
	...
	doc.Find("#sas_word").Each(func(_ int, sas *goquery.Selection) {
		fmt.Fprintln(stdOut, "Suggestion:")
		l := []string{}
		sas.Find("span > a").Each(func(_ int, w *goquery.Selection) {
			l = append(l, w.Text())
		})
